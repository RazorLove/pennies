...
//    Litecoin and modified for Pennies.
//
//    Parameters: lookup (int) - How many blocks to look into the past.
//    Returns: Estimated Pennies network hash rate (uint64_t)
//
...
// WM - Implementation of getnetworkhashps for Pennies
Value getnetworkhashps(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "getnetworkhashps\n"
            "Returns an estimate of the Pennies network hash rate.");
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Pennies is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Pennies is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Pennies is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Pennies is downloading blocks...");
